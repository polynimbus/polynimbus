...
accounts=`/opt/polynimbus/api/v1/account/list.sh aws |grep -vxFf ~/.polynimbus/settings/aws/list-databases.blacklist`
...
for vendor in `/opt/polynimbus/api/v1/all/list-vendors.sh database |grep -v aws`; do
